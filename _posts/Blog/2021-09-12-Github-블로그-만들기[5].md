...
열어보시면 저렇게 **title**이랑 **url** 있는거 보이시죠????

**title**은 말 그대로 **제목**인데,  저기에 입력한대로 눈에 보여요!
...
일케 **navigation.yml** 파일에서 **url 에 설정했던 위치로 이동**하죠???? 짱이죠?? 짱신기하죠????
...
를 추가해볼게용! (여기서 **url**은 저기 위에서 만든 **'[키킼.md]'**파일의 **permalink**랑 **똑같이** 해주셔야해요! 똑같이 안하면.. 이따가 보여드림...)
