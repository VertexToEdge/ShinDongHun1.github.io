...
그러나 detach 혹은 clear를 통해 **영속성 컨텍스트에서 member가 제외**되거나, **영속성 컨텍스트 자체가 초기화** 되었다면 **둘의 동일성을 보장받을 수 없으므로** 위의 코드의 결과는 **false**가 나오게 된다. (참고로 em.close()를 하게되면 Session/EntityManager is closed 라는 오류가 발생한다)
